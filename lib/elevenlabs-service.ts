...
export class ElevenLabsService {
  ...
  private async getSignedUrl(agentId: string): Promise<string> {
    ...
    const response = await fetch(`${config.elevenlabs.apiBaseUrl}/convai/conversation/get_signed_url?agent_id=${agentId}`, {
      headers: {
        'xi-api-key': config.elevenlabs.apiKey || '',
  ...
  private attemptReconnect() {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      }, 5000 * Math.pow(2, this.reconnectAttempts - 1)); // Backoff exponencial
    } else {
